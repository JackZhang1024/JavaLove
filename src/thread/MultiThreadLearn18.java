...
public class MultiThreadLearn18 {
    ...
    private static void sellFruits() {
        ...
        Boss boss1 = new Boss("小张", basket);
        Boss boss2 = new Boss("小李", basket);
        boss1.start();
        boss2.start();
    }

    private static void sellFruitsByThreadLocal() {
        BossThreadLocal boss3 = new BossThreadLocal("小张");
        BossThreadLocal boss4 = new BossThreadLocal("小李");
        boss3.start();
        boss4.start();
    }
    ...
    static class Basket {
        ...
        public void sell() {
            while (!fruits.isEmpty()) {
                ...
                System.out.println(Thread.currentThread().getName() + "卖出水果 " + fruit);
            }
        }
    }
    ...
    // 这块不用new 而是使用get()方法的目的是重用而不是重新开辟新的内存空间来
